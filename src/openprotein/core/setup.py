...
class GlobalConfiguration(metaclass=_Restricted):
    # TODO: support user define config
    # TODO: limit attribute
    # TODO: check toml contents

    # _lib = Tree()
    # __slots__ = ('_backend', '_device', '_component')
    def __init__(self):
        # self._backend = "pt"
        # self._device = "cpu"
        self._lib = Tree()
    ...
    def set_content(self, **kwargs):
        for key, item in kwargs.items():
            try:
                ...
                raise AttributeError(f"the {key} is not a valid attribute.")
            # assert "_" + key.lower() in self.__slots__, "the content is not a valid attribute."
...
# class _Components(type):
#     _data = GlobalConfiguration()
#     def __new__(cls, name, bases, attrs):
